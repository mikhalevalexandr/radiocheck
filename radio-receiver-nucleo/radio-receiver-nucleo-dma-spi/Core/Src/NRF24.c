...
uint8_t NRF24_ReadReg(uint8_t addr)
{
  uint8_t dt=0, cmd;
  CSN1_ON;
  HAL_SPI_TransmitReceive_DMA(&hspi1,&addr,&dt,1);
  if (addr!=STATUS)//если адрес равен адрес регистра статус то и возварщаем его состояние
  {
    cmd=0xFF;
  HAL_SPI_TransmitReceive_DMA(&hspi1,&cmd,&dt,1);
  }
  CSN1_OFF;
  return dt;
}
...
void NRF24_WriteReg(uint8_t addr, uint8_t dt)
{
  ...
  CSN1_ON;
  HAL_SPI_Transmit_DMA(&hspi1,&addr,1);//отправим адрес в шину
  HAL_SPI_Transmit_DMA(&hspi1,&dt,1);//отправим данные в шину
  CSN1_OFF;
}
...

{
  CSN1_ON;
  HAL_SPI_Transmit_DMA(&hspi1,&addr,1);//отправим адрес в шину
  HAL_SPI_Receive_DMA(&hspi1,pBuf,bytes);//отправим данные в буфер
  CSN1_OFF;
}
...

{
  ...
  HAL_SPI_Transmit_DMA(&hspi1,&addr,1);//отправим адрес в шину
  DelayMicro(1);
  HAL_SPI_Transmit_DMA(&hspi1,pBuf,bytes);//отправим данные в буфер
  CSN1_OFF;
}
